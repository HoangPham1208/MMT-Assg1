            ...
            if message_type == 'register':
                ...
                for index_file in self.files:
                    if index_file['file_name'] == peer_file_info:
                        ...
                        peer_file_info = peer_file_info[0] + \
                            ' (1).' + peer_file_info[1]
                        message_to_client += '\n Duplicate file name in server directory. \n Name is changed to ' + peer_file_info
                        break
                self.files.insert(0, dict(
                    zip(self.fileMetaData, [str(self.host), peer_file_info, str(datetime.now())])))
            ...
            if is_client_host_in_server:
                for file in self.files:
                    if file['host_name'] == host_name:
                        file_metadata = [file['host_name'],
                                         file['file_name'], file['date-added']]
                        file_lists.append(
                            dict(zip(file_metadata, self.fileMetaData)))
                return file_lists
