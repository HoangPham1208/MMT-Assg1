...
class CentralizedServer(Thread):
    ...
    def run(self):
        while True:            
            ...
            if message_type == 'register':
                ...
                self.semaphore.acquire()
                client_message = self.client_register(request[1], request[2])
                client.send(pickle.dumps(client_message))
                self.semaphore.release()
                ...
                for index_file in self.files:
                    if index_file['file_name'] == peer_file_info:
                        ...
                        break
                self.files.insert(0, dict(zip(self.fileMetaData, [str(self.host), peer_file_info, str(datetime.now())])))
            ...
            if is_client_host_in_server:
                for file in self.files:
                    if file['host_name'] == host_name:
                        file_metadata = [file['host_name'], file['file_name'], file['date-added']]
                        file_lists.append(dict(zip(file_metadata, self.fileMetaData)))
                return file_lists
    ...
    def client_register(self, client_host_name, client_password, client_addr, client_port):
        ...
        self.clientHost.append(dict(zip(\
                                            ('host_name', 'host_password', 'host_addr', 'host_port'), \
                                            (client_host_name, str(client_password), client_addr, client_port)\
                                            )\
                                        )\
                                   )
        return 'OK'
