...
class CentralizedServer(Thread):
    def __init__(self, maximum_client_number):
        ...
        self.files = []
        self.clientHost = []
        self.clientMetaData = ['host_name', 'host_password', 'host_addr', 'host_port', 'host_live']
    ...
    def run(self):
        while True:            
            ...
            if message_type == 'register':
                ...
                client_message = self.client_register(request[1], request[2], client_addr[0], client_addr[1])
                ...
                try:
                    ...
                    client.send(pickle.dumps('The server is not found your requested hostname'))
                self.semaphore.release()
                ...
                for index_file in self.files:
                    if index_file['file_name'] == file_name_at_server:
                        ...
                        file_name_at_server = file_name_at_server[0] + ' (1).' + file_name_at_server[1]
                        message_to_client += '\n Duplicate file name in server directory. \n Name is changed to ' + file_name_at_server
                        break
                self.files.insert(0, dict(zip(self.fileMetaData, [str(self.host), file_name_at_server, str(datetime.now())])))
    ...
    def list_of_files(self, host_name):
        ...
        if host_name == 'localhost':
            ...
            is_client_host_in_server = [host_name == client_host_name['host_name'] for client_host_name in self.clientHost]
            if is_client_host_in_server:
                for file in self.files:
                    if file['host_name'] == host_name:
                        file_data = [file['host_name'], file['file_name'], file['date-added']]
    ...
    def client_register(self, client_host_name, client_password, client_addr, client_port):
        ...
        self.clientHost.append(dict(zip(    self.clientMetaData, 
                                            (client_host_name, str(client_password), client_addr, client_port, False)\
                                       )\
                                    )\
                               )
        return 'OK'
